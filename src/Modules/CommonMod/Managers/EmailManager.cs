...
<p>验证码有效期为5分钟。</p>";
        await smtp.SendAsync(email, "【IAM】注册验证码", html);
    }
...
<h3>
    <span style='padding:8px;color:white;background-color: rgb(0, 90, 226); border-radius: 5px;'>
        ...
        await smtp.SendAsync(email, "【IAM】注册结果", html);
    }
...
<p>验证码有效期为5分钟。</p>";
        await smtp.SendAsync(email, "【IAM】登录验证码", html);
    }
}
